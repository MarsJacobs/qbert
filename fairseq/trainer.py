...
class Trainer(object):
    ...
    def _build_optimizer(self):
        ...
        #MSKIM add weight decay to clip value
        clip_val = []
        clip_valn = []
        weight = []
        for name, param in self.model.named_parameters():
            if not param.requires_grad:
                continue
            elif 'clip_val' in name:
                if 'clip_valn' not in name:    
                    clip_val.append(param)   
                else:
                    clip_valn.append(param) 
            else:
                weight.append(param)
        #MSKIM clip val weight decay
        params = [{'params' : weight},
                  {'params' : clip_val, 'weight_decay' : self.args.clip_wd},
                  {'params' : clip_valn, 'weight_decay' : self.args.clip_wd}]
        ...
        self._lr_scheduler = lr_scheduler.build_lr_scheduler(self.args, self.optimizer) # self.optimizer returns self._optimizer
    ...
    def train_step(self, samples, run = None, raise_oom=False): 
